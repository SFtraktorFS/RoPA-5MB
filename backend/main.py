...
def startup_event():
    # Ensure all new columns exist on already-created tables before anything else runs
    run_migrations()
    db = SessionLocal()
...
async def approve_ropa_record(
    ...
    return {
        "status": "success",
        "message": f"ROPA record {approval_data.status}",
        "data": updated_record,
    }
