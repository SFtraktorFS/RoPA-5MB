...
export default function Home() {
  ...
  const consentCount = ropaRecords.filter(
    (r) => r.legal_basis === "consent",
  ).length;
  ...
  const getRoleDescription = () => {
    switch (user.role) {
      case "Admin":
        return "คุณมีสิทธิ์เข้าถึงการจัดการระบบทั้งหมด รวมถึงการจัดการผู้ใช้งานและบันทึก RoPA ทุกรายการ";
      case "DPO":
        return "คุณสามารถตรวจสอบและติดตามความปลอดภัยของข้อมูล รวมถึงการเรียกดูรายงานสรุปทั้งหมด";
      case "Data Owner":
        return "คุณสามารถจัดการบันทึก RoPA ของหน่วยงานที่คุณรับผิดชอบและสร้างรายการใหม่ได้";
      default:
        return "";
    }
  };
        ...
        <div className="w-full max-w-4xl rounded-[3rem] border border-white/80 bg-white/70 p-8 md:p-14 shadow-2xl shadow-blue-900/10 backdrop-blur-2xl">
          <div className="grid gap-12">
            <div className="space-y-6 text-center">
              <div
                className={`mx-auto inline-flex items-center px-4 py-1.5 rounded-full border text-xs font-bold uppercase tracking-widest ${getRoleBadgeColor()}`}
              >
                {user.role} Role
              </div>
              <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {["Admin", "Data Owner"].includes(user.role) && (
                <a
                  href="/create"
                  className="flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-6 py-5 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition-all hover:bg-blue-700 hover:shadow-blue-600/30 hover:-translate-y-0.5"
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
              ...
              {["Admin", "DPO"].includes(user.role) && (
                <a
                  href="/dashboard"
                  className="flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-6 py-5 text-sm font-bold text-slate-900 shadow-sm transition-all hover:bg-slate-50 hover:-translate-y-0.5"
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
