...
export default function RecordsPage() {
  ...
  const API_BASE =
    process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3340";
  ...
  const handleApplyFilter = async () => {
    ...
    try {
      ...
      if (filterValues.legal_basis)
        params.append("legal_basis", filterValues.legal_basis);
      if (filterValues.status) params.append("status", filterValues.status);
      if (filterValues.retention_period)
        params.append("retention_period", filterValues.retention_period);

      const response = await fetch(
        `${API_BASE}/ropa/filter?${params.toString()}`,
        {
          headers: {
  ...
  const handleDeleteRecord = async (recordId: number) => {
    ...
    const confirmed = window.confirm(
      "ยืนยันการลบบันทึกนี้? การลบจะไม่สามารถกู้คืนได้",
    );
    if (!confirmed) return;
  ...
  const handleEditChange = (
    ...
    setEditForm((prev) => ({
      ...prev,
      [name]: name === "retention_period" ? parseInt(value) || 0 : value,
    }));
  };
  ...
  const handleApproveSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/ropa/${selectedRecord.id}/approve`,
        {
          method: "POST",
          headers: {
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <Link
              href="/main"
              className="mb-2 inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-blue-600 transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="none"
            ...
            <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-8 shadow-2xl text-gray-700 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-lg text-slate-900">
                    {selectedRecord.legal_basis === "consent"
                      ? "Consent"
                      : "Not Consent"}
                  </p>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-lg text-slate-900">
                    {selectedRecord.retention_period} ปี
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-semibold text-slate-500 uppercase">
                ...
                <div className="space-y-1 pt-2">
                  ...
                  <p className="text-slate-600">
                    {selectedRecord.expiration_date
                      ? new Date(
                          selectedRecord.expiration_date,
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 px-4 py-6">
            <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl text-gray-700">
              <h3 className="text-2xl font-bold text-slate-900 mb-6">
                {selectedRecord.status === "inactive"
                  ? "เปิดใช้งาน RoPA อีกครั้ง"
                  : approvalForm.status === "inactive"
                    ? "ระงับการใช้งาน RoPA"
                    : "ดำเนินการตรวจสอบ RoPA"}
              </h3>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={() =>
                        setApprovalForm((f) => ({ ...f, status: "active" }))
                      }
                    ...
                    <button
                      onClick={() =>
                        setApprovalForm((f) => ({ ...f, status: "inactive" }))
                      }
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setApprovalForm((f) => ({ ...f, reason: e.target.value }))
                    }
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-500 focus:bg-white outline-none transition"
            ...
            <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl text-gray-600">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    เลือกตัวกรองแล้วกดยืนยันเพื่อดูผล
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">
                    เวลาเก็บรักษา(เช่น น้อยกว่า3ปี)
                  </label>
                  <input
                    type="number"
                    name="retention_period"
