...
export default function Dashboard() {
  ...
  const activePercent =
    ropaRecords.length === 0
      ? 0
      : Math.round((activeCount / ropaRecords.length) * 100);
  const consentPercent =
    ropaRecords.length === 0
      ? 0
      : Math.round((consentCount / ropaRecords.length) * 100);

  // Category breakdown
  const categoryCounts: Record<string, number> = {};
  ropaRecords.forEach((r) => {
    categoryCounts[r.data_category] =
      (categoryCounts[r.data_category] || 0) + 1;
  });
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="max-w-md w-full text-center space-y-6 p-10 rounded-[2.5rem] bg-white shadow-xl border border-slate-100">
          ...
          <h2 className="text-2xl font-bold text-slate-900">
            เข้าถึงแดชบอร์ดไม่ได้
          </h2>
          <p className="text-slate-600">
            เฉพาะ Admin และ DPO เท่านั้นที่สามารถเข้าถึงสถิติภาพรวมได้
          </p>
          <button
      ...
      <div className="mx-auto max-w-7xl space-y-8">
                ...
                <ProgressBar
                  ...
                  value={
                    ropaRecords.length === 0
                      ? 0
                      : Math.round(
                          (ropaRecords.filter((r) => r.status === "pending")
                            .length /
                            ropaRecords.length) *
                            100,
                        )
                ...
                <ProgressBar
                  ...
                  value={
                    ropaRecords.length === 0
                      ? 0
                      : Math.round(
                          (ropaRecords.filter((r) => r.status === "expired")
                            .length /
                            ropaRecords.length) *
                            100,
                        )
                ...
                <ProgressBar
                  ...
                  value={
                    ropaRecords.length === 0
                      ? 0
                      : Math.round(
                          (ropaRecords.filter((r) => r.status === "inactive")
                            .length /
                            ropaRecords.length) *
                            100,
                        )
                  ...
                  topCategories.map(([cat, count], idx) => (
                    <ProgressBar
                      ...
                      value={
                        ropaRecords.length === 0
                          ? 0
                          : Math.round((count / ropaRecords.length) * 100)
                      }
        ...
        {!isLoading && ropaRecords.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-900">
              ยังไม่มีข้อมูลสำหรับดู
            </h2>
            <p className="mt-2 text-slate-600 max-w-md">
              เพิ่มข้อมูล ROPA ก่อนเพื่อดูสถิติและการวิเคราะห์
            </p>
            <a
              href="/create"
...
function StatCard({ title, value, desc, icon, color = "blue" }: any) {
  const colorClasses: any = {
    ...
    emerald:
      "from-emerald-50/80 to-emerald-100/40 border-emerald-100 text-emerald-600",
    amber: "from-amber-50/80 to-amber-100/40 border-amber-100 text-amber-600",
    violet:
      "from-violet-50/80 to-violet-100/40 border-violet-100 text-violet-600",
  };

  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border bg-gradient-to-br ${colorClasses[color]} p-5 shadow-lg transition-all hover:-translate-y-1 hover:shadow-xl`}
    >
